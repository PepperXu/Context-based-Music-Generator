...
public class MouseStrax : MonoBehaviour {
	...
	void Update () {
        if (Input.GetMouseButton(0))
        {
            ...
            float note_on = (minor?minorScale[(int)(Mathf.Floor((transform.position.x+4f) * 2f) % 8f)]:majorScle[(int)(Mathf.Floor((transform.position.x+4f) * 2f) % 8f)]) + Mathf.Floor(transform.position.y+1*2) * 12f + note;
